...
def scan(root: Path) -> dict:
    ...
    for key, (path, node, name) in sorted(functions.items()):
        ...
        def resolve(text, aliases=aliases, name=name, module=module):
            first, _, rest = text.partition(".")
            targets = []
            if first in aliases:
        ...
        for item in nodes:
            ...
            if isinstance(item, ast.Call):
                ...
                calls.append(
                    {"line": item.lineno, "target": target or "<dynamic>", "resolved": resolved}
                )
                leaf = imported_name(target, aliases).rsplit(".", 1)[-1]
                if leaf in COMPILERS or leaf in {"exec", "eval", "compile"}:
                    dynamic_sites.append(
                        {"source_id": key, "line": item.lineno, "call": ast.unparse(item)}
                    )
                if leaf in NUMERICAL_METHODS or target.startswith(
                    ("np.linalg.", "np.random.", "math.")
                ):
                    numerical.add(f"numerical_call:{leaf}")
        decorators = [ast.unparse(d) for d in node.decorator_list]
        decorated = any(
            imported_name(symbol(d), aliases).rsplit(".", 1)[-1] in COMPILERS
        ...
        arguments = [
            ...
            for arg, default in zip(
                [*node.args.posonlyargs, *node.args.args], defaults, strict=True
            )
        ]
        arguments += [
            {
        ...
        rows.append(
            {
                ...
                "compiled_decorator": decorated,
                "arguments": arguments,
                "variadic": {
                    "args": node.args.vararg.arg if node.args.vararg else None,
                ...
                "classification": "compiled_numerical"
                if decorated
                else "numerical_candidate"
                if numerical
                else "orchestration_candidate",
                "calls": calls,
                "references": sorted(references),
                "proposed_phase": phase(path),
                "engine_equivalent": "not_verified",
                "platform_integrated": "not_verified",
                "service_verified": "not_verified",
    ...
    for path, tree in trees.items():
        ...
        for item in local_nodes(tree):
            if isinstance(item, ast.Call) and imported_name(symbol(item.func), aliases).rsplit(
                ".", 1
            )[-1] in COMPILERS | {"exec", "eval", "compile"}:
                dynamic_sites.append(
                    {
                        "source_id": f"{path}::<module>",
                        "line": item.lineno,
                        "call": ast.unparse(item),
                    }
                )
            if isinstance(item, (ast.Name, ast.Attribute)) and isinstance(item.ctx, ast.Load):
                module_references.append(
                    {"path": path, "line": item.lineno, "reference": symbol(item)}
                )
    ...
    for row in rows:
        ...
        row["reachability"] = (
            "static_reference" if row["referenced_by"] else "entry_or_dynamic_or_unreferenced"
        )
    return {
        "schema": SCHEMA,
        "source_commit": commit,
        "source_tree": git(root, "rev-parse", "HEAD^{tree}"),
        "modified_tracked_python": sorted(
            set(git(root, "diff", "--name-only", "HEAD").splitlines()) & set(production)
        ),
        "untracked_python": sorted(
            p
            for p in git(root, "ls-files", "--others", "--exclude-standard", "*.py").splitlines()
            if not EXCLUDED.intersection(Path(p).parts)
        ),
        "scope": {
            "tracked": "*.py",
            "excluded_path_parts": sorted(EXCLUDED),
            "evidence": "static_only",
            "unresolved_dispatch": "retained_not_excluded",
        },
        "files": files,
        "functions": rows,
        "dynamic_sites": sorted(dynamic_sites, key=lambda x: (x["source_id"], x["line"])),
        "module_references": sorted(
            module_references, key=lambda x: (x["path"], x["line"], x["reference"])
        ),
    }
...
def main():
    ...
    parser.add_argument(
        "--check", type=Path, help="Fail on any source/inventory drift, not just HEAD changes"
    )
    ...
    if args.check:
        ...
        if old != result:
            raise SystemExit(
                "Platform coverage inventory drift; regenerate and review the changed entries"
            )
    if args.output:
        ...
        for key, value in sorted(result.items()):
            rendered = (
                (
                    "[\n"
                    + ",\n".join(
                        json.dumps(row, ensure_ascii=False, sort_keys=True) for row in value
                    )
                    + "\n]"
                )
        ...
        json.dumps(
            {
                ...
                "dynamic_sites": len(result["dynamic_sites"]),
