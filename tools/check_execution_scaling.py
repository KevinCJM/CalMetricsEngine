...
def measure(function, loops):
    ...
    for _ in range(loops):
        result = function()
    return (
        (time.perf_counter_ns() - begin) / loops,
        result,
        (time.process_time_ns() - cpu_begin) / loops,
    )
...
def main():
    ...
    for name, kernel, shape, layout in [
        ("tensor_small", "affine", [8, 2, 3], "C"),
        ("tensor_medium", "affine", [252, 16, 8], "C"),
        ("tensor_large", "affine", [2520, 64, 8], "C"),
        ("tensor_reverse", "affine", [252, 16, 8], "reverse"),
        ("iteration_medium", "relaxation", [4096], "C"),
        ("iteration_large", "relaxation", [262144], "C"),
    ]:
        for cpu in [1, 2, 4]:
    ...
    for case in cases:
        ...
        ref = reference(case)
        engine, cpp = native(case, x)
        def njit(ref=ref, x=x):
            return ref(x)
        serial_ref = references()[case["kernel"]] if case["cpu"] > 1 else ref
        def serial(serial_ref=serial_ref, x=x):
            return serial_ref(x)
        serial()
        for _ in range(5):
            left, right = cpp(), njit()
        for actual, expected in zip(left.outputs, right, strict=True):
        ...
        result = {
            "case": case,
            "timing": paired_ratio(ca, sa if serial_wins else na),
            "njit_variant": lane,
            "njit_parallel_ns": na if case["cpu"] > 1 else [],
            "njit_serial_ns": sa if sa else na,
            "audit": left.audit,
            "plan": left.plan.metadata(),
            "cpp_cpu_ns": ccpu,
