...
def reference(case):
    ...
    def long(x):
        ...
        for i in range(x.size):
            v = ((((x[i]*2.+1.)*3.-2.)*4.)+1.)
            values[i], mask[i] = v, v > 0.
        return values, mask
    @njit(nogil=True)
    def branched(x):
        values, mask = np.empty(x.size), np.empty(x.size, np.bool_)
        for i in range(x.size):
            a, b = ((x[i]*2.+1.)*3.)-2., ((x[i]*4.-3.)*2.)+1.
            v = (a+b)*2.+3.
            values[i], mask[i] = v, v > 0.
        return values, mask
    @njit(nogil=True)
...
def worker(case, backend, probe_path):
    x = np.linspace(-3., 4., case['n'])
    if case['layout'] == 'reverse': x = x[::-1]
    if case['layout'] == 'nan_heavy':
        x[::7], x[1::127], x[2::127] = np.nan, np.inf, -np.inf
    x.flags.writeable = False
    expression = {'long': '((((x*2+1)*3-2)*4)+1)',
                  'branched': '((((x*2+1)*3-2)+((x*4-3)*2+1))*2+3)',
                  'affine': '(x*2+1)'}[case['kernel']]
    ...
    if backend == 'njit':
        ...
        def setup():
            ...
            graph = GraphCompiler({'x': {'kind': 'vector', 'axes': ['asset'], 'shape': ['N']}}).compile(
                {'value': expression, 'mask': f'{expression}>0'})
    ...
    for a, b in zip(actual, (expected, expected > 0), strict=True): np.testing.assert_array_equal(a, b)
    audit = None if backend == 'njit' else result.audit
    if audit: build = audit['engine_build_id']
    del actual, result
    for _ in range(5): function()
    start()
    retained = None
    for _ in range(11): retained = function()
    warm_stats = stop()
    del retained
    print(json.dumps({'build_id': build, 'audit': audit, 'setup_heap': setup_stats,
        'cold_heap': cold_stats, 'warm_heap': warm_stats,
        'peak_rss': resource.getrusage(resource.RUSAGE_SELF).ru_maxrss}), flush=True)
    if probe:
    ...
    for line in sys.stdin:
        ...
        for _ in range(loops): result = function()
...
def main():
    ...
    args = p.parse_args()
    if args.worker_case:
        worker(json.loads(args.worker_case), args.backend, str(args.heap_probe) if args.heap_probe else None)
        return
    ...
    for case in CASES:
        ...
        def command(backend, heap=False):
            env = dict(os.environ, PYTHONPATH=str(args.before if backend == 'before' else args.after))
            command = [sys.executable, str(Path(__file__).resolve()), '--worker-case', json.dumps(case), '--backend', backend]
            if heap:
        ...
        try:
            ...
            for repeat in range(args.repeats):
                ...
                for backend in order:
                    ...
                    timings[backend].append(stats['ns']); cpu[backend].append(stats['cpu_ns'])
        finally:
            for proc in workers.values():
                if proc.poll() is None:
                    proc.stdin.write('stop\n'); proc.stdin.flush()
                if proc.wait(timeout=30): raise RuntimeError('measurement worker failed')
        heaps = {}
        if args.heap_probe:
            for backend in timings:
                cmd, env = command(backend, True)
                run = subprocess.run(cmd, env=env, text=True, capture_output=True, check=True, timeout=180)
                heaps[backend] = json.loads(run.stdout)
        result = {'case': case, 'metadata': metadata, 'timing_ns': timings, 'cpu_ns': cpu, 'heap': heaps,
        ...
        args.output.write_text(json.dumps({'schema':'pointwise-region-performance-1', 'cases':CASES,
            ...
            'memory_limits':['malloc request bytes; excludes CPython arena subdivisions and mmap',
        ...
        print(case['id'], 'after/before',round(result['after_before']['median'],3),
              'after/NJIT',round(result['after_njit']['median'],3), flush=True)
