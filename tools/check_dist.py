"""Validate built archives without importing code from the checkout (Python 3.11+)."""
...
PYTHON_FILES = {
    ...
    "my_ctools/_api.py",
    "my_ctools/py.typed",
    "my_ctools/cal_std_mean.py",
    "my_ctools/cal_cpr.py",
    "my_ctools/cal_max_dd.py",
    "my_ctools/cal_longest_dd_recover.py",
    "my_ctools/cal_all_largest_indicators.py",
    "my_ctools/cal_all_longest_indicators.py",
    "my_ctools/cal_rolling_gain_loss.py",
}
...
def check_wheel(path: Path, version: str) -> None:
    with zipfile.ZipFile(path) as archive:
        ...
        if len(extensions) != 1 or not extensions[0].startswith("my_ctools/_core."):
            raise ValueError(f"{path.name}: expected exactly one _core extension")
        if any(
        ...
        if metadata["Name"].lower().replace("_", "-") != "my-ctools":
...
def check_sdist(path: Path, version: str) -> None:
    ...
    required = {
        ...
        "README.md",
        "cpp/bindings.cpp",
        "cpp/include/my_ctools/common.hpp",
        "cpp/include/my_ctools/calendar.hpp",
        "cpp/include/my_ctools/kernels.hpp",
