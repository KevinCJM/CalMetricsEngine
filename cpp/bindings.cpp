...
void validate_array(const py::array &array, int dimensions, const char *name) {
  if (!array.dtype().equal(py::dtype::of<T>())) {
    throw py::type_error(std::string(name) +
                         " must use the exact native dtype");
  }
  if (array.ndim() != dimensions) {
    throw py::value_error(std::string(name) +
                          " has an incorrect number of dimensions");
  }
  ...
  for (int axis = 0; axis < dimensions; ++axis) {
    if (array.strides(axis) % static_cast<py::ssize_t>(sizeof(T)) != 0) {
      throw py::value_error(std::string(name) +
                            " has an unsupported byte stride");
    }
  }
  binding::validate_owner(array);
}
...
engine::VectorView<T> vector_view(const py::array &array, const char *name) {
  ...
  return {
      ...
      static_cast<std::ptrdiff_t>(array.strides(0) /
...
engine::MatrixView matrix_view(const py::array &array) {
  ...
  return {
      ...
      static_cast<std::ptrdiff_t>(array.strides(1) /
                                  static_cast<py::ssize_t>(sizeof(double))),
  };
}
...
engine::VectorView<std::int64_t>
date_view(const py::array &array, std::size_t rows, bool sorted = false) {
...
py::array_t<double> mean_standard_deviation(const py::array &values,
                                            int n_threads) {
  const auto view = matrix_view(values);
  const auto threads = checked_threads(n_threads);
  py::array_t<double> output(
      {py::ssize_t(2), static_cast<py::ssize_t>(view.cols)});
  auto *result = output.mutable_data();
  {
    py::gil_scoped_release release;
...
py::array_t<double> cpr(const py::array &types, const py::array &values,
                        int n_threads) {
  const auto view = matrix_view(values);
...
py::array_t<std::int64_t> longest_recovery(const py::array &values,
                                           int n_threads) {
  const auto view = matrix_view(values);
  const auto threads = checked_threads(n_threads);
...
py::tuple max_drawdown(const py::array &values, const py::array &date_array,
                       int n_threads) {
  const auto view = matrix_view(values);
  ...
  {
    py::gil_scoped_release release;
    engine::max_drawdown(view, days, result, output_dates, recovery_data,
                         threads);
  }
...
py::object streak(const py::array &values, const py::array &date_array,
...

  {
    py::gil_scoped_release release;
    const auto kernel =
        longest ? engine::longest_streak : engine::largest_streak;
    kernel(view, days, code == "positive", result, period_data, starts, ends,
           threads);
  }
...
int parse_months(const std::string &code) {
  ...
  if (!std::regex_search(code, match, pattern)) {
    throw py::value_error(
        "i_code must contain a positive period such as 1M, 3M or 1Y");
  }
...
py::tuple rolling(const std::string &code, const py::array &values,
  ...
  if (starts.size != view.cols || ends.size != view.cols) {
    throw py::value_error(
        "start_idx and end_idx lengths must equal the number of columns");
  }
  for (std::size_t col = 0; col < view.cols; ++col) {
    ...
    if (!inactive && !empty &&
        (starts[col] < 0 || ends[col] < starts[col] ||
...
PYBIND11_MODULE(_native, module) {
  ...
  module.def("build_info", [] {
    ...
    info["thread_backend"] = "persistent_cpp_native_scheduler";
    info["cxx_standard"] = 17;
    info["execution_backend"] = "pybind11_aot";
    info["graph_execution_backend"] = "native_graph_interpreter";
    info["scheduler_backend"] = "bounded_thread_process_pool";
    info["scheduler_scope"] = "process_wide_cpp_cpu_admission";
    info["legacy_finance_scheduler"] = "shared_native_scheduler";
    info["coroutine_backend"] = "asyncio_orchestration_only";
    info["process_transport"] = "shared_memory_or_serialized_small_input";
    info["graph_input_memory_policy"] = "product_major_contiguous_zero_copy";
    info["runtime_jit"] = 0;
    info["compiler_backend"] = "cpp_restricted_ast";
    info["planner_backend"] = "cpp";
    info["native_scheduler_backend"] = "persistent_cpp_threads_processes";
    info["native_worker_runtime"] = "standalone_no_python";
    info["python_role"] = "numpy_binding_async_adapter";
    info["input_memory_policy"] = "exact_dtype_strided_zero_copy";
    return info;
  });

  module.def("cal_std_mean", &standard_deviation, py::arg("input").noconvert(),
             py::kw_only(), py::arg("n_threads") = 1);
  module.def("cal_std_mean_simd", &mean_standard_deviation,
             py::arg("input").noconvert(), py::kw_only(),
             py::arg("n_threads") = 1);
  module.def("cal_cpr", &cpr, py::arg("f_type").noconvert(),
             py::arg("funds_value").noconvert(), py::kw_only(),
             py::arg("n_threads") = 1);
  module.def("cal_longest_dd_recover", &longest_recovery,
             py::arg("funds_val").noconvert(), py::kw_only(),
             py::arg("n_threads") = 1);
  module.def("cal_max_dd", &max_drawdown, py::arg("funds_val").noconvert(),
             py::arg("day_arr").noconvert(), py::kw_only(),
             py::arg("n_threads") = 1);
  module.def(
      "cal_all_largest_indicators",
      [](const py::array &values, const py::array &days,
         const std::string &mode,
         int threads) { return streak(values, days, mode, threads, false); },
  ...
  module.def(
      ...
      [](const py::array &values, const py::array &days,
         const std::string &mode,
         int threads) { return streak(values, days, mode, threads, true); },
