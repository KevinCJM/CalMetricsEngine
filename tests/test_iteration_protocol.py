...
from calmetrics_engine import AdaptiveScheduler, GraphCompileError, GraphCompiler
...
def outputs(expression):
    return {
        "value": expression,
        "status": f"iteration_status({expression})",
        "count": f"iteration_count({expression})",
        "residual": f"iteration_residual({expression})",
    }


@pytest.mark.parametrize("lane", ["single", "thread", "process_inline", "process_shared"])
@pytest.mark.parametrize("policy", ["raise", "isolate"])
def test_scalar_iteration_capture_keeps_computed_value(lane, policy):
    binding = {"a": "iterate(iterate_x+1,x,0,1)"}
    graph = GraphCompiler({"x": {"kind": "value"}}).compile(
        {"first": "a", "second": "iterate(iterate_x+a,x,0,1)"},
        root_bindings=[binding, binding],
        error_policy=policy,
    )
    graph = pickle.loads(pickle.dumps(graph))
    x = np.array(2.0)
    starts, ends = np.array([0, 0], np.int64), np.array([1, 1], np.int64)
    saved = []
    with AdaptiveScheduler(cpu_budget=2, config=configuration(lane)) as engine:
        prepared = engine.prepare_execution(graph, {"x": x}, starts, ends) if lane == "single" else None
        for initial in (2.0, -4.0):
            x[...] = initial
            results = [engine.execute(graph, {"x": x}, starts, ends)]
            if prepared is not None:
                results.append(prepared.run_snapshot())
            for result in results:
                for field, expected in zip(result.outputs, (initial + 1, 2 * initial + 1), strict=True):
                    for actual in field.values:
                        assert actual == expected
                saved.append((result, initial))
    for result, initial in saved:
        assert result.named_outputs["second"].values[0] == 2 * initial + 1
...
def test_native_iteration_shapes_lanes_and_cse(lane, shape):
    spec = (
        ...
        if not shape
            ...
            {"kind": "vector", "axes": ["asset"], "shape": ["N"]}
            if len(shape) == 1
            else {"kind": "matrix", "axes": ["model", "asset"], "shape": ["M", "N"]}
            if len(shape) == 2
            else tensor(temporal=False)
        )
    )
    initial = np.full(shape, 8.0)
    ...
    with AdaptiveScheduler(cpu_budget=2, config=configuration(lane)) as engine:
        result = engine.execute(
            graph, {"x": initial}, np.array([0, 0], np.int64), np.array([1, 1], np.int64)
        )
    fields = result.named_outputs
    expected_count = 23 if initial.size else 1
    for row in range(2):
        np.testing.assert_allclose(
            fields["value"].values[row], initial * 0.5**expected_count, rtol=0, atol=0
        )
    ...
    np.testing.assert_array_equal(initial, np.full(shape, 8.0))


@pytest.mark.parametrize(
    "expression,status,count,value",
    [
...
def test_iteration_stopping_is_separate_from_execution_error(expression, status, count, value):
    ...
    with AdaptiveScheduler(cpu_budget=1) as engine:
        result = engine.execute(
            graph, {"unused": np.ones(1)}, np.array([0], np.int64), np.array([1], np.int64)
        )
    fields = result.named_outputs
...
def test_iteration_more_than_eight_captures_and_nested_scope():
    ...
    expression = (
        "iterate(iterate_x*0.5+" + "+".join(f"sum({key})*0.5" for key in variables) + ",0,1e-8,100)"
    )
...
def test_iteration_invalid_limits_fail_even_with_failed_initial(suffix):
    graph = GraphCompiler({"x": "series"}).compile(
        f"iterate(iterate_x,1/std(x,0),{suffix})", error_policy="isolate"
    )
    with AdaptiveScheduler(cpu_budget=1) as engine:
        with pytest.raises((ValueError, RuntimeError), match="INVALID_PARAMETER"):
            engine.execute(
                graph, {"x": np.ones(4)}, np.array([0], np.int64), np.array([4], np.int64)
            )
...
def test_iteration_rejects_shape_change_and_non_native_callback():
    ...
    for expression in [
        "iterate(sum(iterate_x),x,1e-6,20)",
        "iterate(callback(iterate_x),x,1e-6,20)",
    ]:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (1.0, "iterate_x/0+sum(gather(y,idx))"),
    ],
)
def test_numerical_stop_preserves_independent_body_structure_validation(
    policy, lane, initial, body
):
    compiler = GraphCompiler(
        {"x": "series", "y": "series", "idx": {"kind": "series", "dtype": "int64"}}
    )
    ...
    with AdaptiveScheduler(cpu_budget=2, config=configuration(lane)) as engine:
        with pytest.raises((ValueError, RuntimeError), match="INDEX_OUT_OF_BOUNDS"):
            engine.execute(
                graph,
                {"x": np.array([initial]), "y": np.ones(1), "idx": np.array([7], np.int64)},
...
def test_nonfinite_initial_state_returns_numerical_failure_without_invalid_payload_use(policy):
    ...
    with AdaptiveScheduler(cpu_budget=1) as engine:
        result = engine.execute(
            graph, {"x": np.array([np.nan])}, np.array([0], np.int64), np.array([1], np.int64)
        )
    assert result.named_outputs["status"].values[0] == 5
    assert result.named_outputs["count"].values[0] == 0


@pytest.mark.parametrize("factor", [0.5, 1.01])
