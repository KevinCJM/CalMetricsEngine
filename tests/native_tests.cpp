...
int main() {
    ...
    try {
        ...
        mc::rolling_gain({nullptr, 0, 2}, starts, ends, nullptr, 1, outputs, 2);
        for (std::size_t k = 0; k < outputs.size(); ++k) {
        ...
        ends[0] = ends[1] = 0;
        mc::rolling_gain({one_row, 1, 2}, starts, ends, day, 1, outputs, 2);
        for (std::size_t k = 0; k < outputs.size(); ++k) {
        ...
        mc::max_drawdown({nullptr, 0, 2}, nullptr, result, dates, periods, 2);
        require(std::isnan(result[0]) && periods[0] == mc::not_recovered, "empty max drawdown");
        mc::longest_recovery({nullptr, 0, 2}, periods, 2);
        require(periods[0] == 0 && periods[1] == 0, "empty recovery");
        mc::column_statistics({nullptr, 0, 2}, result, nullptr, 2);
