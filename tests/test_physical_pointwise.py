...
def schema(shape):
    return {"kind": {1: "vector", 2: "matrix", 3: "tensor"}[len(shape)],
...
def test_fused_roots_shared_ancestors_duplicates_and_snapshots(shape, layout):
    ...
    with AdaptiveScheduler(cpu_budget=1) as engine:
        prepared = engine.prepare_execution(graph, {"x": x}, np.array([0], np.int64), np.array([1], np.int64))
...
def test_tensor_temporal_rows_keep_slice_boundaries():
    x = np.arange(5*3*7.).reshape(5, 3, 7)
    kind = {"kind": "tensor", "axes": ["time", "path", "asset"], "shape": ["T", "P", "N"]}
    graph = GraphCompiler({"x": kind}).compile({"value": "x*2+1", "mask": "x>3"})
    with AdaptiveScheduler(cpu_budget=1) as engine:
        result = engine.execute(graph, {"x": x}, np.array([0, 1, 4], np.int64), np.array([0, 4, 5], np.int64))
...
def test_tensor_partition_uses_one_budget_and_disjoint_root_slots(cpu, temporal):
    ...
    assert sum(chunk["direct_output_bytes"] for chunk in result.audit["native_chunks"]) == x.size * 9
...
def test_register_chain_preserves_rounding_order_roots_and_strides(ops, sides, layout):
    base = np.resize(np.array([1.-2.**-27, -0., 0., np.nan, np.inf, -np.inf, 2.]), (3, 5, 9))
    x = base if layout == 'C' else base[:, ::-1, ::-1] if layout == 'negative' else np.broadcast_to(base[:, :, :1], base.shape)
    x.flags.writeable = False
    ...
    graph = GraphCompiler({'x': schema(x.shape), 'c': 'scalar', 'd': 'scalar'}).compile(
        {'first': first, 'second': second, 'mask': f'({second})>0', 'input': 'x', 'duplicate': second})
    ...
    with AdaptiveScheduler(cpu_budget=1) as engine:
        result = engine.execute(graph, {'x': x}, np.array([0], np.int64), np.array([1], np.int64), parameters={'c': c, 'd': d})
    for field, expected in zip(result.outputs, [a, b, b>0, x, b], strict=True):
        ...
        if expected.dtype == np.float64:
            zero = expected == 0
            np.testing.assert_array_equal(np.signbit(field.values[0][zero]), np.signbit(expected[zero]))
...
def test_register_predicate_only_and_scalar_operand_order(predicate, reverse):
    x = np.array([np.nan, -np.inf, -0., 0., 1., np.inf])
    expression = f'0{predicate}x' if reverse else f'x{predicate}0'
    graph = GraphCompiler({'x': schema(x.shape)}).compile({'mask': expression})
    compare = {'==': np.equal, '!=': np.not_equal, '<': np.less, '<=': np.less_equal,
               '>': np.greater, '>=': np.greater_equal}[predicate]
...
@pytest.mark.parametrize('layout', ['inner_reverse', 'all_reverse', 'outer_reverse',
                                   'transpose', 'zero', 'step', 'singleton'])
def test_register_chain_strided_runs_cross_cache_tiles(layout):
    base = np.arange(7*17*19., dtype=np.float64).reshape(7,17,19)
    layouts = {'inner_reverse': base[:,::-1,::-1], 'all_reverse': base[::-1,::-1,::-1],
               'outer_reverse': base[::-1], 'transpose': base.transpose(2,0,1),
               'zero': np.broadcast_to(base[:,:1,:1],base.shape), 'step': base[:,:,::2],
...
def test_iteration_finiteness_proof_rechecks_mutable_coefficients():
    ...
    x = np.resize(np.array([8., -8., -0., np.finfo(float).max]), 4097)
    x.flags.writeable = False
    ...
    saved = []
    with AdaptiveScheduler(cpu_budget=1) as engine:
        for factor in [.5, 2., -1., np.inf, 0., np.nan, 1., -.5, 1.00001]:
            params[0] = factor
            state = x.copy()
            ...
            if np.isfinite(factor) and abs(factor) <= 1:
                assert result.audit['native_chunks'][0]['operator_workspace_capacity_bytes'] < x.nbytes
...
def test_segmented_chain_keeps_external_consumers_and_liveness(layout, shape, retained):
    base = np.resize(np.array([np.nan, np.inf, -np.inf, -0., 0., 1.-2.**-27, 2.]), shape)
    x = {'C': base, 'negative': base[:, ::-1, ::-1],
         'zero': np.broadcast_to(base[:, :, :1], shape),
         'transpose': base.transpose(2, 0, 1), 'step': base[:, :, ::2]}[layout]
    x.flags.writeable = False
    ...
    params = np.array([1.+2.**-27, -1.])
    with AdaptiveScheduler(cpu_budget=1) as engine:
        prepared = engine.prepare_execution(graph, {'x': x}, np.array([0], np.int64),
            np.array([1], np.int64), parameters=params)
        saved = []
        for a, b in [(1.+2.**-27, -1.), (-.5, 0.)]:
            params[:] = a, b
            with np.errstate(invalid='ignore', over='ignore'):
                first_value = x*a
                ...
                if retained: expected += [first_value, first_value-7, second_value]
            ...
            for output, wanted in zip(result.outputs, expected, strict=True):
                ...
                if wanted.dtype == np.float64:
                    np.testing.assert_array_equal(np.signbit(actual[wanted == 0]), np.signbit(wanted[wanted == 0]))
                assert not np.shares_memory(actual, x)
...
def test_segmented_branch_fusion_transports_roots_and_shared_dependencies(lane):
    ...
    graph = GraphCompiler(types).compile({'sum': f'({lhs})+({rhs})',
        'shared': f'(({lhs})+({rhs}))*2+3', 'mask': f'(({lhs})+({rhs}))*2+3>0'})
    config = PlannerConfig(thread_work_units=1 if lane == 'thread' else 1e100,
        ...
        min_rows_per_worker=1, shared_memory_threshold_bytes=1 if lane == 'process_shared' else 1<<30)
    with AdaptiveScheduler(cpu_budget=2 if lane != 'single' else 1, config=config) as engine:
        result = engine.execute(graph, {'x': x, 'y': y}, np.zeros(4, np.int64), np.ones(4, np.int64))
    value = ((x*2+1)*3-2)+((y*4-3)*2+1)
    for field, expected in zip(result.outputs, [value, value*2+3, value*2+3>0], strict=True):
        for actual in field.values: np.testing.assert_array_equal(actual, expected)
...
def test_long_generic_chain_preserves_masks_scalar_expressions_and_branches(policy):
    ...
    expressions = {'value': 'absolute(((x+y)*2+1)-y)*3+2',
                   'mask': 'absolute(((x+y)*2+1)-y)*3+2>=4',
                   'scalar_expr': '(x*(a+1)+2)*3-1', 'mask_branch': 'x>0',
                   'raw': 'x'}
    graph = GraphCompiler(types).compile(expressions, error_policy=policy)
    expected = np.abs(((x+y)*2+1)-y)*3+2
    with AdaptiveScheduler(cpu_budget=1) as engine:
        result = engine.execute(graph, {'x': x, 'y': y}, np.array([0], np.int64), np.array([1], np.int64), parameters=scalar)
    for field, wanted in zip(result.outputs, [expected, expected>=4, (x*(scalar[0]+1)+2)*3-1, x>0, x], strict=True):
