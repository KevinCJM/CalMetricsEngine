...
- 在预算和布局通过后、计划返回 READY 前，对冻结输入执行一次原生数值域检查。直接算子检查结果，图检查实际执行的输入、参数、常量、canonical 算子及作用域/迭代诊断结果，涵盖嵌套作用域；诊断执行关闭会隐藏中间值的逐元素融合。算术 Inf、次正规结果和超域整数返回 `UNREPRESENTABLE_VALUE`，不能让后续 bool/归约掩盖溢出。掩码极值的全 NaN 哨兵保留既有文本类别。此检查只在导出计划构造时运行，属于额外的数值执行/工作区成本；纯符号预算阶段仍不求值。
