...
- 在预算和布局通过后、计划返回 READY 前，对冻结输入执行一次原生数值域检查。直接算子检查结果，图按 canonical 节点检查实际执行的操作数与结果，涵盖嵌套作用域；诊断执行关闭会隐藏中间值的逐元素融合。算术 Inf、次正规结果和超域整数返回 `UNREPRESENTABLE_VALUE`，不能让后续 bool/归约掩盖溢出。掩码极值的全 NaN 哨兵保留既有文本类别。此检查只在导出计划构造时运行，属于额外的数值执行/工作区成本；纯符号预算阶段仍不求值。
...
- bool、int64、NaN 和执行错误分别表示。Excel 数值运算只接受绝对值不超过 `999999999999999` 的 int64；不把大整数悄悄转 double。输入 Inf、次正规数拒绝，数值域检查还拒绝实际节点的算术溢出/次正规结果。该检查不模拟 Excel 在每个耦合内核内部的全部浮点步骤，仍不能据普通样本通过推断所有极值组合已支持。
