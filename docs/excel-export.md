...
- 在预算和布局通过后、计划返回 READY 前，对冻结输入执行一次原生数值域检查。直接算子检查操作数和结果，图检查实际执行的输入、参数、常量、canonical 算子及作用域/迭代诊断结果，涵盖嵌套作用域；诊断执行关闭会隐藏中间值的逐元素融合。算术 Inf、次正规结果和超域整数返回 `UNREPRESENTABLE_VALUE`，不能让后续 bool/归约掩盖溢出。掩码极值的全 NaN 哨兵保留既有文本类别。此检查只在导出计划构造时运行，属于额外的数值执行/工作区成本；纯符号预算阶段仍不求值。
...
- 正态密度在绝对值超过 40 时按原生 binary64 的确定零尾部处理，避免 Excel 平方溢出；二分宽度超出有限浮点范围时保留原生停止比较为 false 的语义，不改变不收敛或非有限函数值错误。
- 偏度和超额峰度另复用 canonical 均值/总体方差，保守约束偏差幂次累加及标准化分母，拒绝内部上溢或下溢风险；不是将原算法替换为归一化算法。缺失和常量样本保留原有契约，额外扫描只发生于导出准入。
- period、window、最大迭代数等布局参数必须能从常量、显式标量参数或可静态传播表达式确定。任意数据归约决定布局而不能确定上界时返回 `UNKNOWN_BOUND`。用户在工作簿中修改这些参数后显示 `LAYOUT_STALE`，需要重新导出。
- bool、int64、NaN 和执行错误分别表示。Excel 数值运算只接受绝对值不超过 `999999999999999` 的 int64；不把大整数悄悄转 double。输入 Inf、次正规数拒绝，数值域检查还拒绝实际节点的算术溢出/次正规结果。有限数绝对值上界为 `1.797693134862315e308`，同时约束输入、常量和执行结果，避免接近 DBL_MAX 的冻结参考在 xlsx 数字序列化时被舍入成 Inf。Excel 公式结果上限高于直接输入字面量上限，本后端通过短整数和二进制幂重建，允许 `1e308`；不能误套用直接输入上限 `9.99999999999999e307`。该检查不模拟 Excel 在每个耦合内核内部的全部浮点步骤，仍不能据普通样本通过推断所有极值组合已支持。
...
Results 各列为：名称、Excel 公式值、冻结 C++ 参考值、绝对误差、比较状态。浮点采用导出前确定的 `atol + rtol*abs(reference)`；离散值与错误状态严格比较。容差本身须在可表示域内，且冻结参考对应的容差乘积与总和不能溢出；否则在 READY 前明确拒绝。默认容差适用于当前夹具，病态问题仍须由调用方按算法验收；不能事后扩大容差掩盖失败。
...
`.github/workflows/excel.yml` 配置 Linux LibreOffice 差分及证据产物。PR #5 已执行该工作流；每次新提交仍须独立检查该提交的 CI 结论。测试同时核对原始公式/输入身份、冻结 C++ 值、dtype、逐位置状态和表内比较，不能仅凭可编辑的 MATCH 标签通过。
